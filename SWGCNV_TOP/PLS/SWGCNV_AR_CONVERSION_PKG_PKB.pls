...
/*
and     p_ship_address_ref			=   lpad (RTRIM (LTRIM (SUBSTR (SUBSTR (hzcas.orig_system_reference
						   					 			   		   		   ,LENGTH(hzca.orig_system_reference)+2
						   												   ),1
																    ,DECODE (INSTR (hzcas.orig_system_reference,'-HEADER')
                    					 									,0,10
                    					 									,INSTR (hzcas.orig_system_reference,'-HEADER')
							         		 						         - (LENGTH (hzca.orig_system_reference)+2)
										                   )
									                 )
								               )
...
BEGIN
   ...
   BEGIN
		...
		l_ra_int_lines_rec.trx_date						:=	cur_trx_rec.trx_date;
   		l_ra_int_lines_rec.sales_order					:=	'Legacy Sales Order#: ' ||
								        					LTRIM(RTRIM(cur_trx_rec.sales_order));
...
--Billto Address Id
	...
	END IF;
--Payment Terms
IF l_ra_int_lines_rec.cust_trx_type_name NOT IN ('DDCONV PAYMENT','DDCONV CREDIT MEMO') THEN
	BEGIN
...
END IF;
--Salesrep id
	SELECT swgcnv_salesrep(p_legacy_system,cur_trx_rec.primary_salesrep_number)
...
--Updated on 09/30/05 -- Commented out by Ashok on 02/23/07 for ARS01
    ...
    Swgcnv_Cntrct_Vldt.get_maps_and_details( in_sacs_org_s         	    => p_sales_center
...
-- 02/28/07 -- Ashok -- modified for ARS01 conversion Begin
    ...
    Swgcnv_Cntrct_Vldt.get_maps_and_details( in_sacs_org_s         	    => p_sales_center
            ...
            IF l_status_c != Swgcnv_Cntrct_Vldt.G_STS_SUCCESS THEN
                l_new_item_code   := NULL;
                l_error_msg	      :=	p_legacy_system||' -'||LTRIM(RTRIM(cur_trx_rec.item_code))  ||
								  		'Sub Code'									   ||
								  		cur_trx_rec.item_sub_code					   ||
										'Sales Center'								   ||
										p_sales_center                                 ||
...
*/
  EXCEPTION
	 WHEN OTHERS THEN
		  ...
		  Fnd_File.put_line(Fnd_File.LOG,'error while calling ar conversion procedure for trx_number: '||cur_trx_rec.interface_line_attribute1||': '||SQLERRM);
  END;
  END LOOP; --cur_customer_trx
   Fnd_File.put_line(Fnd_File.LOG,'no of records are inserted : '||l_cust_trx_cnt);
...
END		insert_ra_lines;
--
PROCEDURE swgcnv_update_duedate(ou_errmsg_s      	OUT   	VARCHAR2
							   ,ou_errcode_n     	OUT   	NUMBER
							   ,p_sales_center     	IN      VARCHAR2
							   )
IS
...
and    a.cust_trx_type_id       in (1020,1021,1022,1400);
l_cnt              NUMBER := 0;
...
EXCEPTION
	WHEN OTHERS THEN
		...
		VALUES ('AR Update Duedate'
			   ,p_sales_center
			   ,l_err_message);
	Fnd_File.put_line(Fnd_File.LOG,'Error : '||SQLERRM);
END Swgcnv_Update_Duedate;
--
PROCEDURE Swgcnv_Post_Update1 (ou_errmsg_s      	OUT   	VARCHAR2
							   ,ou_errcode_n     	OUT   	NUMBER
							   ,p_sales_center     	IN      VARCHAR2
							   )
IS
...
--
PROCEDURE Swgcnv_Post_Update2 (ou_errmsg_s      	OUT   	VARCHAR2
							   ,ou_errcode_n     	OUT   	NUMBER
							   ,p_sales_center     	IN      VARCHAR2
							   )
IS
...
--
PROCEDURE Swgcnv_ar_Trx_Number(ou_errmsg_s      	OUT   	VARCHAR2
							  ,ou_errcode_n     	OUT   	NUMBER
							  ,p_sales_center		IN		VARCHAR2
                                            ,p_legacy_system      IN          VARCHAR2
							  )
...
LOOP
	...
	LOOP
		...
		,	   	 interface_line_attribute1        = j.sales_center||p_legacy_system||'_'||l_header_cnt
...
--
PROCEDURE Swgcnv_ar_Preupdates(ou_errmsg_s      	OUT   	VARCHAR2
							  ,ou_errcode_n     	OUT   	NUMBER
							  ,p_division			IN      VARCHAR2
							  ,p_sales_center		IN		VARCHAR2
							  )
...
--
PROCEDURE swgcnv_ar_preconv_reports(ou_errmsg_s      	OUT   	VARCHAR2
							       ,ou_errcode_n     	OUT   	NUMBER
							       ,p_sales_center		IN		VARCHAR2
							       )
IS
...
FROM     swgcnv_dd_ar_history
WHERE    tax_status  	NOT IN ('UI','UN','UP','UR','UC','U')
AND      sales_center 	= p_sales_center
GROUP BY sales_center
;
...
,		 SUM(NVL(amount,0) + NVL(attribute10,0)) total_amt
FROM     swgcnv_dd_ar_history
WHERE    tax_status  	IN ('UI','UN','UP','UR','UC','U')
AND      sales_center 	= p_sales_center
GROUP BY sales_center
;
...
END IF;
Fnd_File.put_line(Fnd_File.output,' ');
Fnd_File.put_line(Fnd_File.output,'Report2 : Total Transactons for the '||p_sales_center||' Sales center Transaction Type wise');
Fnd_File.put_line(Fnd_File.output,' ');
...
END IF;
Fnd_File.put_line(Fnd_File.output,' ');
Fnd_File.put_line(Fnd_File.output,'Report3 : Total Transactons for the '||p_sales_center||' Sales center without Customers');
Fnd_File.put_line(Fnd_File.output,' ');
...
Fnd_File.put_line(Fnd_File.output,'Report5 : AR Balances for '||p_sales_center||' Sales center for Statements');
Fnd_File.put_line(Fnd_File.output,' ');
...
FOR n IN f
LOOP
	Fnd_File.put_line(Fnd_File.output,RPAD(' ',5,' ')||RPAD('Exclude UI,UN,UP,UC,U  ',22,' ')||' '||RPAD(n.sales_center,12,' ')||' '||RPAD(n.cnt,11,' ')||'  '||LPAD(n.total_amt,14,' '));
...
--
PROCEDURE swgcnv_ar_preconv_reports1(ou_errmsg_s      	OUT   	VARCHAR2
							       ,ou_errcode_n     	OUT   	NUMBER
							       ,p_sales_center		IN		VARCHAR2
							       )
IS
...
--
procedure swgcnv_seq_prog(out_errbuf_s	    OUT	VARCHAR2
...
begin
--1st
	 	l_request_id1 	           :=	Fnd_Request.submit_request('SWGCNV'
		...
		Fnd_File.put_line(Fnd_File.LOG,l_request_id1);
	     LOOP
			l_status_b1 	       :=	Fnd_Concurrent.wait_for_request(l_request_id1
														   , 15
													       , 15
													       , l_phase1
													       , l_status1
...
--2nd
		IF 	l_dev_phase1		   =	'COMPLETE' AND
			...
			l_request_id1		   <>	0
		THEN
		 	l_request_id2 	       :=	Fnd_Request.submit_request('SWGCNV'
		...
		Fnd_File.put_line(Fnd_File.LOG,l_request_id2);
	     LOOP
			l_status_b2 	       :=	Fnd_Concurrent.wait_for_request(l_request_id2
														   , 15
													       , 15
													       , l_phase2
													       , l_status2
...
--3rd
		IF 	l_dev_phase2	       =	'COMPLETE' AND
			...
			l_request_id2		   <>	0
		THEN
		 	l_request_id3 	       :=	Fnd_Request.submit_request('SWGCNV'
		...
		Fnd_File.put_line(Fnd_File.LOG,l_request_id3);
	     LOOP
			l_status_b3 	       :=	Fnd_Concurrent.wait_for_request(l_request_id3
														   , 15
													       , 15
													       , l_phase3
													       , l_status3
...
--4th
		IF 	l_dev_phase3	       =	'COMPLETE' AND
			...
			l_request_id3		   <>	0
		THEN
		 	l_request_id4 	       :=	Fnd_Request.submit_request('SWGCNV'
		...
		Fnd_File.put_line(Fnd_File.LOG,l_request_id4);
	     LOOP
			l_status_b4 	       :=	Fnd_Concurrent.wait_for_request(l_request_id4
														   , 15
													       , 15
													       , l_phase4
													       , l_status4
...
--5th
		IF 	l_dev_phase4	        =	'COMPLETE' AND
			...
			l_request_id4		    <>	0
		THEN
		 	l_request_id5 	        :=	Fnd_Request.submit_request('SWGCNV'
		...
		Fnd_File.put_line(Fnd_File.LOG,l_request_id5);
	     LOOP
			l_status_b5 	       :=	Fnd_Concurrent.wait_for_request(l_request_id5
														   , 15
													       , 15
													       , l_phase5
													       , l_status5
...
--6th
		IF 	l_dev_phase5	       =	'COMPLETE' AND
			...
			l_request_id5		   <>	0
		THEN
		 	l_request_id6 	       :=	Fnd_Request.submit_request('SWGCNV'
		...
		Fnd_File.put_line(Fnd_File.LOG,l_request_id6);
	     LOOP
			l_status_b6 	:=	Fnd_Concurrent.wait_for_request(l_request_id6
														   , 15
													       , 15
													       , l_phase6
													       , l_status6

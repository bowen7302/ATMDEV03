...
+=============================================================================*/
   ...
   IS
      ...
      AND     hzca.cust_account_id         =   p_cust_account_id
      */ -- Commented Pankaj. Code Tuning
      -- Commented out by Ashok Krishnamurthy on 08/03/07 BEGIN
      /*
      AND      p_ship_address_ref           =   lpad (RTRIM (LTRIM (SUBSTR (SUBSTR (hzcas.orig_system_reference                                                            ,LENGTH(hzca.orig_system_reference)+2),1,DECODE (INSTR (hzcas.orig_system_reference,'-HEADER'),0,10                                                                          ,INSTR (hzcas.orig_system_reference,'-HEADER') - (LENGTH (hzca.orig_system_reference)+2))))),9,'0')
      */
      -- Commented out by Ashok Krishnamurthy on 08/03/07 END
      -- Added by Ashok Krishnamurthy on 08/03/07 BEGIN
      /* -- Commented Pankaj. Code Tuning
      AND      p_ship_address_ref          =   decode( instr(hzcas.orig_system_reference,'HEADER',1,1),
                                                      0,substr(hzcas.orig_system_reference,instr(hzcas.orig_system_reference,'-',1,4)+1),
                                                      substr(replace(hzcas.orig_system_reference,'-HEADER'),instr(hzcas.orig_system_reference,'-',1,4)+1)
                                                    )
      -- Added by Ashok Krishnamurthy on 08/03/07 END
      ;
      */ -- Commented Pankaj. Code Tuning
      --AND     hzcas.orig_system_reference like   l_ship_address_ref;
   ...
   BEGIN
      ...
      l_shipto_address_ref := 'DD'||'-'|| p_legacy_system   ||'%'||
                              -- p_sales_center             ||'%'||
                              p_customer_ref ||'%'|| l_shipto_number ||'%';
   ...
   BEGIN
    ...
    SELECT count( distinct h.sales_center)
    INTO   l_sc_n
    FROM   swgcnv_dd_ar_history h,
           swgcnv_dd_cb_prestaging_cust cust
    WHERE  cust.customer_number = h.orig_system_bill_customer_ref;
...
---
   ...
   IS
      ...
      --MTS 20129 AND     a.division               =   b.division
      AND     a.cust_trx_type_name     <>  'PAYMENT'
      AND     a.orig_system_bill_customer_ref  =  b.legacy_customer_number
      --MTS 2012 AND     a.sales_center           =   b.new_sales_center
     --MTS  2012  AND     b.cust_import_flag       =  'Y'
     --MTS  2012      AND     b.division               =   p_division
      ...
      */
      ...
      AND       hzcsu.site_use_code         =   'BILL_TO'
      --MTS 20129      AND      hzcas.orig_system_reference =   p_bill_address_ref
         ...
         BEGIN
            ...
            IF ( cur_trx_rec.line_type   =    'LINE' )  THEN
               ...
               */  -- Commented To Reduce Function Call Overhead. Pankaj
               ...
               --Inventory item id
               /*
               SELECT   swgcnv_newitem_code(  p_sales_center
               ...
               */
               ...
               /*
               --Updated on 09/30/05 -- Commented out by Ashok on 02/23/07 for ARS01
               ...
               IF l_status_c != Swgcnv_Cntrct_Vldt.G_STS_SUCCESS THEN
                  ...
                  l_error_msg         :=    'RIM -'||LTRIM(RTRIM(cur_trx_rec.item_code))  ||
                                       'Sub Code' || cur_trx_rec.item_sub_code || 'Sales Center' || p_sales_center || 
                                       'Error returned from Get_Maps_And_Details: ' || l_error_msg;
               ...
               END IF;
               */
               -- 02/28/07 -- Ashok -- modified for ARS01 conversion Begin
               ...
               Swgcnv_Cntrct_Vldt.get_maps_and_details
                  ...
                  -- 02/23/07 -- Ashok -- modified for ARS01 conversion End
                  --09/30/05
               ...
               -- Uncommented For RHDIST Conversion
               -- HardCode GL Distributions.
...
--           IF p_sales_center = 'ORM' THEN
--               l_ra_int_dist_segment2_s := '3110';   --MTS ARS07 PROJ# 1641
 --          END IF;
--           IF p_sales_center = 'LIN' THEN
--               l_ra_int_dist_segment2_s := '0520';   --MTS ARS06 PROJ# 1552
--           END IF;
/*          

               ELSIF p_sales_center = 'MNT' THEN 
                  l_ra_int_dist_segment2_s := '4550';
               ELSIF p_sales_center = 'BRW' THEN 
                  l_ra_int_dist_segment2_s := '4512';
               ELSIF p_sales_center = 'BLY' THEN 
                  l_ra_int_dist_segment2_s := '4514';
              ELSIF p_sales_center = 'PLM' THEN 
                  l_ra_int_dist_segment2_s := '4509';                 
              ELSIF p_sales_center = 'WIN' THEN 
                  l_ra_int_dist_segment2_s := '4516';
              END IF;

*/               --MTS 303 ANB1/DSD CONVERSION
               ...
               IF p_sales_center = 'DCH' THEN
                  l_ra_int_dist_segment2_s := '6790';  
               ELSIF p_sales_center = 'DEL' THEN
                  l_ra_int_dist_segment2_s := '6598';
               ELSIF p_sales_center = 'DKN' THEN
                  l_ra_int_dist_segment2_s := '6590';
               ELSIF p_sales_center = 'DBA' THEN
                  l_ra_int_dist_segment2_s := '6592';
               ELSIF p_sales_center = 'DSL' THEN
                  l_ra_int_dist_segment2_s := '6594';
               ELSIF p_sales_center = 'DGL' THEN
                  l_ra_int_dist_segment2_s := '6588';
               ELSIF p_sales_center = 'DLF' THEN
                  l_ra_int_dist_segment2_s := '6597';           
               END IF;
               */ -- Commented For SAGE Acquisition

/*
              IF p_sales_center = 'SLC' THEN  --MTS VOYMTO PROJ# 1646
                  l_ra_int_dist_segment2_s := '3120';
              ELSIF p_sales_center = 'VER' THEN 
              l_ra_int_dist_segment2_s := '3140';
          ELSIF p_sales_center = 'RCF' THEN 
                  l_ra_int_dist_segment2_s := '3170';
              ELSIF p_sales_center = 'SGE' THEN 
                  l_ra_int_dist_segment2_s := '3190';
              ELSIF p_sales_center = 'TWF' THEN 
              l_ra_int_dist_segment2_s := '4980';
              END IF;
*/
            ...
            ELSIF ( cur_trx_rec.line_type   =   'TAX' )  THEN
               ...
               -- Commented For ARS04 Conversion
               --IF cur_trx_rec.tax_status IN ('UI','UP','UR','UN','UC','U') THEN 
               IF cur_trx_rec.tax_status IS  NULL  THEN
...
--               IF p_sales_center = 'ORM' THEN
--                  l_ra_int_tax_segment2_s := '3110';   --MTS ARS07 PROJ# 1641
--               END IF;
--               IF p_sales_center = 'LIN' THEN
--                  l_ra_int_tax_segment2_s := '0520';   --MTS ARS06 PROJ# 1552
--               END IF;
/*
               IF p_sales_center = 'LAP' THEN  --MTS ARS05 PROJ# ???
                  l_ra_int_tax_segment2_s := '4523';
               ELSIF p_sales_center = 'IRV' THEN 
                              l_ra_int_dist_segment2_s := '4526';
               ELSIF p_sales_center = 'RIV' THEN 
                  l_ra_int_tax_segment2_s := '4511';
               ELSIF p_sales_center = 'STA' THEN 
                  l_ra_int_tax_segment2_s := '4511';
               ELSIF p_sales_center = 'FRS' THEN 
                  l_ra_int_tax_segment2_s := '4540';
               ELSIF p_sales_center = 'MNT' THEN 
                  l_ra_int_tax_segment2_s := '4550';
               ELSIF p_sales_center = 'BRW' THEN 
                  l_ra_int_tax_segment2_s := '4512';
               ELSIF p_sales_center = 'BLY' THEN 
                  l_ra_int_tax_segment2_s := '4514';
               ELSIF p_sales_center = 'PLM' THEN 
                  l_ra_int_tax_segment2_s := '4509';              
               ELSIF p_sales_center = 'WIN' THEN 
                  l_ra_int_tax_segment2_s := '4516';
               END IF;
*/             --mts 303 ABN1/DSD CONVERSION
            -- Commented For SAGE ACQ
            /*
               IF p_sales_center = 'DCH' THEN
                  l_ra_int_tax_segment2_s := '6790';  
               ELSIF p_sales_center = 'DEL' THEN
                  l_ra_int_tax_segment2_s := '6598';
               ELSIF p_sales_center = 'DKN' THEN
                  l_ra_int_tax_segment2_s := '6590';
               ELSIF p_sales_center = 'DBA' THEN
                  l_ra_int_tax_segment2_s := '6592';
               ELSIF p_sales_center = 'DSL' THEN
                  l_ra_int_tax_segment2_s := '6594';
               ELSIF p_sales_center = 'DGL' THEN
                  l_ra_int_tax_segment2_s := '6588';
               ELSIF p_sales_center = 'DLF' THEN
                  l_ra_int_tax_segment2_s := '6597';            
            END IF;
               */ -- Commented For SAGE ACQ
         /*     IF p_sales_center = 'SLC' THEN  --MTS VOYMTO PROJ# 1646
                  l_ra_int_tax_segment2_s := '3120';
              ELSIF p_sales_center = 'VER' THEN 
              l_ra_int_tax_segment2_s := '3140';
          ELSIF p_sales_center = 'RCF' THEN 
                  l_ra_int_tax_segment2_s := '3170';
              ELSIF p_sales_center = 'SGE' THEN 
                  l_ra_int_tax_segment2_s := '3190';
              ELSIF p_sales_center = 'TWF' THEN 
              l_ra_int_tax_segment2_s := '4980';
              END IF;
*/
...
--              l_ra_int_tax_segment2_s := l_ra_int_dist_segment2_s;
               INSERT INTO ra_interface_distributions_all
                  (   interface_line_context
                     ,interface_line_attribute1
                     ,interface_line_attribute2
            ...
            SET    ar_proc_flag           =  'Y'
            WHERE  interface_line_attribute1  =   l_ra_int_lines_rec.interface_line_attribute1
            AND    interface_line_attribute2  =   l_ra_int_lines_rec.interface_line_attribute2
            AND    interface_line_context     =  'DD CONVERSION'
            AND    trx_date                   =   l_ra_int_lines_rec.trx_date
            AND    sales_center               =   p_sales_center;
   ...
   IS
      ...
      --and    a.cust_trx_type_id       in (1020,1021,1022,1400);  -- No Longer Using these Transaction Types.
      ...
      LOOP
         ...
         LOOP
            ...
            SET  interface_line_attribute2    =   l_line_cnt
               --  ,interface_line_attribute1      =   j.sales_center||p_legacy_system||'_'||l_header_cnt -- Commented for WO 18998. Conversion Enhancements
               --  ,interface_line_attribute1      =   j.sales_center||p_legacy_system||'-'||j.trx_number -- Added for WO 18998. Conversion Enhancements
               -- Commented For RAM01 Conversion
   ...
   IS
      ...
      --WHERE     sales_center = p_sales_center
      --WHERE     legacy_system  = p_sales_center
      ...
      --WHERE     legacy_system  = p_sales_center
      WHERE    1 = 1
      AND      orig_system_bill_customer_ref NOT IN (/*
                                                     IN
                                                (     SELECT   orig_system_bill_customer_ref
                                                      FROM     swgcnv_dd_ar_history
                                                      WHERE sales_center   =  p_sales_center
      ...
      FROM      swgcnv_dd_ar_history
      WHERE     trxn_status       IS NULL   -- NOT IN ('UI','UN','UP','UR','UC','U') SAGE Acquisition Changes
      --WHERE     sales_center = p_sales_center
      --WHERE     legacy_system  = p_sales_center
      GROUP BY  sales_center;
      ...
      SELECT    sales_center
               ...
               ,SUM(NVL(amount,0) + NVL(attribute10,0)) total_amt
      FROM      swgcnv_dd_ar_history
      WHERE     trxn_status        =  'NA' -- IN ('UI','UN','UP','UR','UC','U'), SAGE Acquisition Changes
      --WHERE     sales_center = p_sales_center
      --WHERE     legacy_system  = p_sales_center
      GROUP BY  sales_center;
   ...
   BEGIN
      ...
      Fnd_File.PUT_LINE(Fnd_File.output,RPAD(' ',30,' ')||'Pre Conversion AR Reports Ver.1');
      ...
      Fnd_File.put_line(Fnd_File.output,'Report1 : Total Transactons for the '||p_sales_center||' Legacy System');
      Fnd_File.put_line(Fnd_File.output,' ');
      ...
      Fnd_File.put_line(Fnd_File.output,' ');
      --Fnd_File.put_line(Fnd_File.output,'Report2 : Total Transactions for the '||p_sales_center||' Sales center Transaction Type');
      Fnd_File.put_line(Fnd_File.output,'Report2 : Total Transactions for the Legacy System by Sales center Trans Type');
      Fnd_File.put_line(Fnd_File.output,' ');
      Fnd_File.PUT_LINE(Fnd_File.output,RPAD(' ',5,' ')||'  '||'Sales Center'||' '||'Transaction Type'||' '||'Trans Status'||'  '||'No of Trans'||'  '||' Trans Amount '||'  '||'TAX Amount'||' '||'  Total Amount  ');
      ...
      Fnd_File.put_line(Fnd_File.output,' ');
      Fnd_File.put_line(Fnd_File.output,'Report3 : Total Transactions for the '||p_sales_center||' Sales center without Customers');
      Fnd_File.put_line(Fnd_File.output,' ');
      ...
      Fnd_File.put_line(Fnd_File.output,'Report4 : Statement Balance for '||p_sales_center||' Sales center');
      Fnd_File.put_line(Fnd_File.output,' ');
      ...
      Fnd_File.put_line(Fnd_File.output,'Report5 : AR Balances for for Legacy System By Sales center for Statements');
      Fnd_File.put_line(Fnd_File.output,' ');
      ...
      FOR n IN f
      LOOP
         Fnd_File.put_line(Fnd_File.output,RPAD(' ',5,' ')||RPAD('Include Trx On Stmt  ',22,' ')||' '||RPAD(n.sales_center,12,' ')||' '||RPAD(n.cnt,11,' ')||'  '||LPAD(n.total_amt,14,' '));
         l_rpt5_cnt := l_rpt5_cnt + 1;
      END LOOP;--i
      
      FOR O IN G
      LOOP
         Fnd_File.put_line(Fnd_File.output,RPAD(' ',5,' ')||RPAD('Exclude from MSI,22,' ')||' '||RPAD(O.sales_center,12,' ')||' '||RPAD(O.cnt,11,' ')||'  '||LPAD(O.total_amt,14,' '));
      END LOOP;--i
   ...
   BEGIN
      ...
      Fnd_File.put_line(Fnd_File.output,' ');
      Fnd_File.put_line(Fnd_File.output,' ');
      --Fnd_File.put_line(Fnd_File.output,'Report1 : Total Transactions for the '||p_sales_center||' Sales center');
      Fnd_File.put_line(Fnd_File.output,'Report1 : Total Transactions for the '||p_sales_center||' Legacy System');
      Fnd_File.put_line(Fnd_File.output,' ');
      Fnd_File.PUT_LINE(Fnd_File.output,RPAD(' ',5,' ')||'Sales Center'||'  '||'No of Trans'||'  '||' Trans Amount '||'  '||'TAX Amount'||' '||'  Total Amount  ');
      Fnd_File.put_line(Fnd_File.output,RPAD(' ',5,' ')||RPAD('=',12,'=')||'  '||RPAD('=',11,'=')||'  '||LPAD('=',14,'=')||'  '||LPAD('=',10,'=')||' '||LPAD('=',16,'='));
      Fnd_File.put_line(Fnd_File.output,' ');
      ...
      Fnd_File.put_line(Fnd_File.output,' ');
      Fnd_File.put_line(Fnd_File.output,'Report2 : Total Transactions for the '||p_sales_center||' Legacy System by Transaction Type/Unbilled - billed status');
      Fnd_File.put_line(Fnd_File.output,' ');
      ...
      --3)
      --Fnd_File.put_line(Fnd_File.output,'Report3 : Total Transactions for the '||p_sales_center||' Sales center Transaction Type');
      Fnd_File.put_line(Fnd_File.output,'Report3 : Total Transactions for the Legaacy Center Transaction Type');
      Fnd_File.put_line(Fnd_File.output,' ');
      ...
      --4)
      Fnd_File.put_line(Fnd_File.output,CHR(12));
      Fnd_File.put_line(Fnd_File.output,'Report4 : Total Transactions for the Legacy Center by Sales center with missing Customers');
      Fnd_File.put_line(Fnd_File.output,' ');
      Fnd_File.PUT_LINE(Fnd_File.output,RPAD(' ',5,' ')||'Legacy Customer'||' '||'Transaction Type'||' '||'Trxn Status'||'  '||'No of Trans'||'  '||' Trans Amount '||'  '||'TAX Amount'||' '||'  Total Amount  ');
      Fnd_File.put_line(Fnd_File.output,RPAD(' ',5,' ')||RPAD('=',15,'=')||' '||RPAD('=',16,'=')||'  '||RPAD('=',10,'=')||'  '||RPAD('=',11,'=')||'  '||LPAD('=',14,'=')||'  '||LPAD('=',10,'=')||' '||LPAD('=',16,'='));
      Fnd_File.put_line(Fnd_File.output,' ');
...
--MTS 20129 process tax when included as separate tax line instead of amount in attribute10
     ...
     FROM  
         ...
         (SELECT MAX(int1.line_number)
           ...
           AND   SIGN(INT1.AMOUNT) = (SELECT SIGN(SUM(int2.attribute10))
                                      ...
                                      AND   int2.line_type = 'TAX')
           AND  sales_center    = IN_SALES_CENTER_S
           AND  division        = IN_DIVISION_ID_N) line_count,
          (SELECT sum(amount)
    ...
    --       WHERE TRX_NUMBER = 'HOU-IBM01-710299401'
     ...
     AND   division     =    in_division_id_n
     AND   line_type    = 'LINE'
     ORDER BY   ABS(amount) desc    --apply to largest
--line_number
      ;
   ...
   BEGIN
      ...
      FOR idx IN 1 .. l_tbl_tax_amt_rec.COUNT LOOP
          ...
          FOR cur_line_tax_amt_rec IN cur_line_tax_amt(l_tbl_tax_amt_rec(idx).tax_rate,l_tbl_tax_amt_rec(idx).trx_number) LOOP
           ...
           --      IF cur_line_tax_amt_rec.line_number = l_tbl_tax_amt_rec(idx).line_count THEN  --LAST TRANSACTION LINE WITH SAME SIGN AS TAX
...
--SGB
    ...
    END IF;
      ...
      BEGIN
         ...
         INSERT INTO SWGCNV_DD_AR_INTERFACE VALUES l_Tbl_Tax_Rec(i);
                                                ...
                                                /*
                                                 l_Tbl_Tax_Rec(i).Cust_Trx_Type_Name
                                                ,l_Tbl_Tax_Rec(i).interface_line_context
                                                ,l_Tbl_Tax_Rec(i).orig_system_bill_customer_ref
                                                ,l_Tbl_Tax_Rec(i).orig_system_bill_address_ref
                                                ,l_Tbl_Tax_Rec(i).orig_system_ship_customer_ref
                                                ,l_Tbl_Tax_Rec(i).orig_system_ship_address_ref
                                                ,l_Tbl_Tax_Rec(i).interface_line_attribute1 
                                                ,l_Tbl_Tax_Rec(i).interface_line_attribute2
                                                ,l_Tbl_Tax_Rec(i).line_type
                                                ,l_Tbl_Tax_Rec(i).trx_date
                                                ,l_Tbl_Tax_Rec(i).gl_date
                                                ,l_Tbl_Tax_Rec(i).trx_number
                                                ,l_Tbl_Tax_Rec(i).line_number
                                                ,l_Tbl_Tax_Rec(i).description
                                                ,l_Tbl_Tax_Rec(i).amount
                                                ,l_Tbl_Tax_Rec(i).currency_code
                                                ,l_Tbl_Tax_Rec(i).term_name
                                                ,l_Tbl_Tax_Rec(i).item_code
                                                ,l_Tbl_Tax_Rec(i).quantity
                                                ,l_Tbl_Tax_Rec(i).uom_code
                                                ,l_Tbl_Tax_Rec(i).quantity_ordered
                                                ,l_Tbl_Tax_Rec(i).unit_selling_price
                                                ,l_Tbl_Tax_Rec(i).unit_standard_price
                                                ,l_Tbl_Tax_Rec(i).reason_code
                                                ,l_Tbl_Tax_Rec(i).tax_rate
                                                ,l_Tbl_Tax_Rec(i).tax_code
                                                ,l_Tbl_Tax_Rec(i).primary_salesrep_number
                                                ,l_Tbl_Tax_Rec(i).comments
                                                ,l_Tbl_Tax_Rec(i).attribute10
                                                ,l_Tbl_Tax_Rec(i).tax_exempt_flag
                                                ,l_Tbl_Tax_Rec(i).tax_exempt_reason_code
                                                ,l_Tbl_Tax_Rec(i).tax_exempt_number
                                                ,l_Tbl_Tax_Rec(i).sales_order
                                                ,l_Tbl_Tax_Rec(i).sales_order_line
                                                ,l_Tbl_Tax_Rec(i).sales_order_date
                                                ,l_Tbl_Tax_Rec(i).sales_order_source
                                                ,l_Tbl_Tax_Rec(i).sales_order_revision
                                                ,l_Tbl_Tax_Rec(i).purchase_order
                                                ,l_Tbl_Tax_Rec(i).purchase_order_revision
                                                ,l_Tbl_Tax_Rec(i).purchase_order_date
                                                ,l_Tbl_Tax_Rec(i).reference_line_context
                                                ,l_Tbl_Tax_Rec(i).reference_line_attribute1
                                                ,l_Tbl_Tax_Rec(i).reference_line_attribute2
                                                ,l_Tbl_Tax_Rec(i).sales_center 
                                                ,l_Tbl_Tax_Rec(i).division 
                                                ,l_Tbl_Tax_Rec(i).attribute1
                                                ,l_Tbl_Tax_Rec(i).attribute2
                                                ,l_Tbl_Tax_Rec(i).tax_status
                                                ,l_Tbl_Tax_Rec(i).item_sub_code
                                                ,l_Tbl_Tax_Rec(i).due_date
                                                ,l_Tbl_Tax_Rec(i).ar_proc_flag
                                                ,l_Tbl_Tax_Rec(i).sub_cust_num
                                            );
   ...
   IS
      ...
      CURSOR   cur_AR_data( in_division_s VARCHAR2
                           ,in_sales_ctr_s VARCHAR2
                           )
      IS
      SELECT ROWID
            ,CUST_TRX_TYPE_NAME
      ...
      */
      ...
      CURSOR   cur_btm_cust(   in_division_s    VARCHAR2
                              ,in_sales_ctr_s   VARCHAR2
                           )
      IS
      SELECT  orig_system_bill_customer_ref cust_num
      FROM    swgcnv_dd_ar_interface
      WHERE   sales_center    =  in_sales_ctr_s
   ...
   BEGIN
      ...
      */
      ...
      FOR cur_btm_cust_rec IN cur_btm_cust(   l_division_s
                                             ,l_sales_center_s
                                          )
      LOOP
         ...
         BEGIN
            ...
            IF l_btm_s IS NOT NULL THEN
               ...
               WHERE   division                        =   l_division_s
               AND     sales_center                    =   l_sales_center_s
               AND     ORIG_SYSTEM_BILL_CUSTOMER_REF   =   cur_btm_cust_rec.cust_num;
...
--Transactions with both positive and negative lines
/*CURSOR dual_sign_csr IS
  SELECT DISTINCT
    AH.TRX_NUMBER,
    INTERFACE_LINE_ATTRIBUTE1,
    AH.ORIG_SYSTEM_SHIP_CUSTOMER_REF
  FROM
...
--   and trx_number = '12201411'
     AND EXISTS (SELECT TRX_NUMBER
              ...
              AND ah1.line_type = 'LINE')) neg
  WHERE
     AH.TRX_NUMBER = NEG.TRX_NUMBER
...
CURSOR inv_csr IS
  SELECT
      nl.trx_number,
      nl.interface_line_attribute1,
      nl.orig_system_ship_customer_ref,
      ah.line_number,
      ah.amount
  FROM 
      swgcnv_fix_neg_inv_lines nl,
      swgcnv_dd_ar_history  ah
  WHERE ah.amount >= 0
  AND   ah.trx_number = nl.trx_number
  AND   ah.orig_system_ship_customer_ref = nl.orig_system_ship_customer_ref
--AND ah.TRX_NUMBER  LIKE 'EDS-ARS06-25879%'
  order by trx_number,orig_system_ship_customer_ref,line_number;
...
CURSOR update_loc_div_csr IS
SELECT DISTINCT 
  ah.orig_system_ship_address_ref ,
  cust.sales_center,
  cust.division
FROM
  swgcnv_dd_ar_history ah,
...
FROM
   swgcnv_dd_ar_history 
where  
   nvl(amount,0) <> (nvl(unit_selling_price,0) * nvl(quantity,0))
...
FROM
--  swgcnv_fix_neg_inv_lines  nl,
    swgcnv_dd_ar_history ar
WHERE cust_trx_type_name  !=  'PAYMENT'
--  nl.trx_number = ar.trx_number
--AND nl.orig_system_ship_customer_ref = ar.orig_system_ship_customer_ref
GROUP BY 
  ar.trx_number,
  ar.interface_line_attribute1,
  ar.orig_system_ship_customer_ref;

CURSOR cycle_day_csr IS
SELECT DISTINCT
  ah.trx_number,
  ah.trx_date,
  bt.customer_number,
  --bt.billing_cycle_day, ah.sales_center, map1.new_code, 
  map.new_code bill_cycle_day
 -- map.old_code  cycle_day
--  SUBSTR(map.new_code,9,2) cycle_day
FROM
  swgcnv_dd_customer_billto bt,
  swgcnv_map map,
  swgcnv_map map1,
  swgcnv_dd_ar_history ah
WHERE
    map.old_code = bt.billing_cycle_day
...
CURSOR fix_tax_trx_lines_csr (in_trx_number_s VARCHAR2, in_bill_cust_ref_s VARCHAR2) IS
     SELECT 
          trx_number,
          line_number,
          line_type,
...
CURSOR billed_unbilled_csr IS
       ...
       FROM
         ...
         swgcnv_dd_cb_prestaging_cust cust
       WHERE
         cust.customer_number = ah.orig_system_bill_customer_ref;
...
/*CURSOR billed_unbilled_csr IS
       SELECT
          ...
          AH.TRX_NUMBER,
          AH.TRX_DATE,
          CASE WHEN CUST.STMT_TYPE = 'P' THEN
               'NA'
          ELSE
              CASE WHEN TRX_DATE <= '31-MAY-12' THEN
                 'NA'
              ELSE
                 NULL
              END
...
UPDATE /*+parallel(ah)*/swgcnv_dd_ar_history ah
...
SET
    orig_system_bill_address_ref  = orig_system_bill_CUSTOMER_ref,
...
/*
--Update references when bill to master doesn't exist in customer file
UPDATE
    swgcnv_dd_ar_history ah
SET
    orig_system_bill_customer_ref = orig_system_ship_address_ref,
    orig_system_bill_address_ref = orig_system_ship_address_ref,
    orig_system_ship_customer_ref = orig_system_ship_address_ref
WHERE 
  NOT EXISTS 
     (SELECT 1
      FROM SWGCNV_DD_CB_PRESTAGING_CUST CUST
...
FOR location_rec IN location_csr LOOP
    UPDATE 
          swgcnv_dd_ar_history
    SET   
          sales_center = location_rec.sales_center,
          division     = location_rec.division
    WHERE
          sales_center = location_rec.legacy_sales_center;
END LOOP;
...
FOR update_loc_div_rec IN update_loc_div_csr LOOP
    UPDATE 
          swgcnv_dd_ar_history
    SET   
          sales_center = update_loc_div_rec.sales_center,
          division     = update_loc_div_rec.division
    WHERE
          orig_system_ship_address_ref  = update_loc_div_rec.orig_system_ship_address_ref; 
END LOOP;
...
FOR dual_sign_rec IN dual_sign_csr LOOP
    ...
    IF dual_sign_rec.trx_total < 0 THEN
       UPDATE 
           swgcnv_dd_ar_history
       SET 
           cust_trx_type_name = 'CREDIT MEMO'
       WHERE 
           trx_number = dual_sign_rec.trx_number
       AND orig_system_ship_customer_ref = dual_sign_rec.orig_system_ship_customer_ref
       AND cust_trx_type_name <> 'CREDIT MEMO'
       AND cust_trx_type_name != 'PAYMENT';
    ELSE
       UPDATE 
           swgcnv_dd_ar_history
       ...
       WHERE 
           trx_number = dual_sign_rec.trx_number
       AND orig_system_ship_customer_ref = dual_sign_rec.orig_system_ship_customer_ref
       AND cust_trx_type_name NOT IN ( 'DEBIT MEMO','INVOICE' )
       AND cust_trx_type_name != 'PAYMENT';
    END IF;
...
/*
FOR dual_sign_rec IN dual_sign_csr LOOP
    INSERT INTO swgcnv.swgcnv_fix_neg_inv_lines
    VALUES 
       (dual_sign_rec.trx_number,
        dual_sign_rec.interface_line_attribute1,
        dual_sign_rec.orig_system_ship_customer_ref);
END LOOP;

  l_message_s := '******************Dual Sign Transactions******************';
  swg_output(l_message_s);

  l_message_s := 'Transaction Type|old_trx_number|new_trx_number|old_line_number|new_line_number|amount';
  swg_output(l_message_s);
    
l_sav_trx_s      := ' ';
l_sav_ship_ref_s := ' ';

FOR inv_rec IN inv_csr LOOP
    IF l_sav_trx_s <> inv_rec.trx_number  or l_sav_ship_ref_s <>  inv_rec.orig_system_ship_customer_ref THEN
       l_sav_trx_s := inv_rec.trx_number;
       l_sav_ship_ref_s :=  inv_rec.orig_system_ship_customer_ref;
       l_line_num_n := 1;
   ELSE
       l_line_num_n := l_line_num_n + 1;
   END IF;
   --dbms_output.put_line('l_line_num_n '||l_line_num_n);
   --dbms_output.put_line('inv_rec.line_number '||inv_rec.line_number);
   --dbms_output.put_line('orig_system_ship_customer_ref '||INV_REC.orig_system_ship_customer_ref);
    UPDATE 
           swgcnv_dd_ar_history
    SET    
           line_number = l_line_num_n,
           interface_line_attribute2  = l_line_num_n,
           cust_trx_type_name = 'INVOICE'
          --reference_line_attribute1  = to_char(sysdate,'DD-MON-YYYY HH24:MI:SS')
    WHERE
          inv_rec.trx_number = trx_number
    AND    inv_rec.line_number = line_number
    --AND    inv_rec.amount = amount
    AND    inv_rec.orig_system_ship_customer_ref = orig_system_ship_customer_ref
   -- AND    rowid = inv_rec.row_id
    AND    amount >= 0;
    l_message_s := 'Invoice'||'|'||inv_rec.trx_number||'|'||inv_rec.trx_number||'|'||inv_rec.line_number||'|'||l_line_num_n||'|'||inv_rec.amount;
    swg_output(l_message_s);
    
END LOOP;    
l_sav_trx_s := ' ';
l_sav_ship_ref_s := ' ';
FOR cm_rec IN cm_csr LOOP
    IF l_sav_trx_s <> cm_rec.trx_number  or l_sav_ship_ref_s <>  cm_rec.orig_system_ship_customer_ref THEN
       l_sav_trx_s := cm_rec.trx_number;
       l_sav_ship_ref_s :=  cm_rec.orig_system_ship_customer_ref;
       l_line_num_n := 1;
   ELSE
       l_line_num_n := l_line_num_n + 1;
   END IF;
    ...
    SET    
           line_number = l_line_num_n,
           interface_line_attribute2  = l_line_num_n,
           trx_number = trx_number||'C',
           interface_line_attribute1 = interface_line_attribute1||'C',
           cust_trx_type_name = 'CREDIT MEMO'           
    WHERE
           cm_rec.trx_number = trx_number
    AND    cm_rec.line_number = line_number
    AND    cm_rec.amount = amount
    AND    cm_rec.orig_system_ship_customer_ref = orig_system_ship_customer_ref
    AND    amount < 0;
    
    l_message_s := 'Credit Memo'||'|'||cm_rec.trx_number||'|'||cm_rec.trx_number||'C'||'|'||cm_rec.line_number||'|'||l_line_num_n||'|'||cm_rec.amount;
    swg_output(l_message_s);
END LOOP;
*/
...
FOR dupe_rec IN dupe_csr LOOP
   ...
   END IF;
    UPDATE swgcnv_dd_ar_history
    SET    
          trx_number = REPLACE(trx_number,'-CM','CM')||'-'||l_dupe_cnt_n,
          interface_line_attribute1 = REPLACE(interface_line_attribute1,'-CM','CM')||'-'||l_dupe_cnt_n
    WHERE
          trx_number = dupe_rec.trx_number
...
FOR fix_date_rec IN fix_date_csr LOOP
      UPDATE swgcnv_dd_ar_history
      SET    
            trx_date = fix_date_rec.trx_date,
            gl_date = fix_date_rec.trx_date,
            due_date = fix_date_rec.trx_date
      WHERE
            trx_number = fix_date_rec.trx_number
...
IF nvl(l_correct_price_n,0) = nvl(incorrect_prc_rec.unit_Selling_price,0)
THEN

  Null;

ELSIF nvl(l_correct_price_n,0) <> nvl(incorrect_prc_rec.unit_Selling_price,0)
THEN
      update swgcnv_dd_ar_history
      set unit_selling_price =   l_correct_price_n , ar_proc_flag = 'N'
...
--UPDATE gl_date based on trx_date and current gl period start date
   SELECT 
       start_date
   INTO 
       l_period_start_date_d
   FROM
       gl_periods
   WHERE  
        period_set_name = 'SWG Calendar'
...
/*   UPDATE SWGCNV_DD_AR_HISTORY
   SET 
     trx_number = trx_number||'-CM',
     interface_line_attribute1 = interface_line_attribute1||'-CM',
     cust_trx_type_name = 'CREDIT MEMO'
   WHERE 
       cust_trx_type_name = 'INVOICE'
   AND AMOUNT < 0;

   l_message_s := 'Update Negative Invoices to CREDIT_MEMO '||SQL%ROWCOUNT||' rows updated';
   swg_output(l_message_s); 
   ...
   UPDATE 
      swgcnv_dd_ar_history
   SET 
      cust_trx_type_name = 'DEBIT MEMO'
   WHERE 
      cust_trx_type_name = 'PAYMENT'
   AND AMOUNT > 0;

   l_message_s := 'Update Positive Payments to DEBIT_MEMO '||SQL%ROWCOUNT||' rows updated';
   swg_output(l_message_s); 
   ...
   l_message_s := 'Update tax_status for unbilled transactions to NULL '||SQL%ROWCOUNT||' rows updated';
   swg_output(l_message_s);          
   ...
   l_message_s := 'Update tax_status for billed transactions to NA '||SQL%ROWCOUNT||' rows updated';
   swg_output(l_message_s);       

--Update tax status for billed and unbilled transactions
FOR cycle_day_rec IN cycle_day_csr LOOP
    IF cycle_day_rec.bill_cycle_day = 'MONTHLY-21' THEN
       IF cycle_day_rec.trx_date <= '20-APR-12' THEN
          l_tax_status_s := 'NA';
       ELSE
          l_tax_status_s := NULL;          
       END IF;
    ELSE
       IF cycle_day_rec.trx_date <= '01-APR-12' THEN
          l_tax_status_s := 'NA';
       ELSE
          l_tax_status_s := NULL;          
       END IF;
    ...
    UPDATE
      swgcnv_dd_ar_history
    SET
      tax_status = l_tax_status_s
    WHERE
      trx_number = cycle_day_rec.trx_number
    AND orig_system_bill_customer_ref = cycle_day_rec.customer_number;
END LOOP;
    ...
    FOR fix_tax_trx_lines_rec IN fix_tax_trx_lines_csr(fix_tax_trx_rec.trx_number ,fix_tax_trx_rec.orig_system_bill_customer_ref) LOOP
     BEGIN
        ...
        SET
           ...
           interface_line_attribute2 = l_line_num_n
        WHERE
           ROWID = fix_tax_trx_lines_rec.ROWID;
        l_line_num_n := l_line_num_n + 1;
...
--dupe transactions
  BEGIN
    ...
    FROM
       swgcnv_dd_ar_history dah
    WHERE
       dah.cust_trx_type_name <> 'PAYMENT'
    GROUP BY dah.cust_trx_type_name, dah.trx_number, dah.line_number
    HAVING COUNT(*) > 1;
  ...
  BEGIN
    ...
    FROM  
        swgcnv_dd_ar_history
    WHERE 
        NVL(interface_line_attribute1,' ') != trx_number;
  EXCEPTION WHEN NO_DATA_FOUND THEN
    l_rec_cnt_n := 0;
  END;
  ...
  BEGIN
   ...
   FROM
       (SELECT DISTINCT
        ...
        FROM
           swgcnv_dd_ar_history)
   GROUP BY trx_number, orig_system_ship_customer_ref,cust_trx_type_name --,trx_date
   HAVING COUNT(*) > 1;
  EXCEPTION WHEN NO_DATA_FOUND THEN
    l_rec_cnt_n := 0;
  END;
...
--
  BEGIN 
   ...
   FROM
        swgcnv_dd_ar_history
   WHERE 
      NVL(cust_trx_type_name,' ') NOT IN ('PAYMENT','INVOICE','CREDIT MEMO','DEBIT MEMO');
  EXCEPTION WHEN NO_DATA_FOUND THEN
    l_rec_cnt_n := 0;
  END;
...
--
  BEGIN
   ...
   FROM
        swgcnv_dd_ar_history
   where 
      NVL(interface_line_context,' ') <> 'DD CONVERSION';
  EXCEPTION WHEN NO_DATA_FOUND THEN
    l_rec_cnt_n := 0;
  END;
...
--
  BEGIN
   ...
   FROM
        swgcnv_dd_ar_history
   WHERE 
      length(trim(translate(orig_system_bill_customer_ref, '0123456789',' '))) IS NOT NULL;
  EXCEPTION WHEN NO_DATA_FOUND THEN
    l_rec_cnt_n := 0;
  END;
...
--
  BEGIN
   ...
   FROM
        swgcnv_dd_ar_history
   WHERE 
      length(trim(translate(orig_system_bill_address_ref, '0123456789',' '))) IS NOT NULL;
  EXCEPTION WHEN NO_DATA_FOUND THEN
    l_rec_cnt_n := 0;
  END;
...
--
  BEGIN
   ...
   FROM
        swgcnv_dd_ar_history
   WHERE 
      length(trim(translate(orig_system_ship_customer_ref, '0123456789',' '))) IS NOT NULL;
  EXCEPTION WHEN NO_DATA_FOUND THEN
    l_rec_cnt_n := 0;
  END;
...
--
  BEGIN
   ...
   FROM
        swgcnv_dd_ar_history
   WHERE 
      length(trim(translate(orig_system_ship_address_ref, '0123456789',' '))) IS NOT NULL;
  EXCEPTION WHEN NO_DATA_FOUND THEN
    l_rec_cnt_n := 0;
  END;
...
--
  BEGIN
   ...
   FROM
        swgcnv_dd_ar_history
   WHERE 
        NVL(reference_line_context,' ') <> 'DD CONVERSION'
   AND  NVL(line_type,' ') <> 'TAX';
  EXCEPTION WHEN NO_DATA_FOUND THEN
    l_rec_cnt_n := 0;
  END;
...
--
  BEGIN
   ...
   FROM
        swgcnv_dd_ar_history
   WHERE 
        NVL(trxn_status,' ') NOT IN ('B','U','NA',NULL)
   AND  NVL(line_type,' ') <> 'TAX';
  EXCEPTION WHEN NO_DATA_FOUND THEN
    l_rec_cnt_n := 0;
  END;
